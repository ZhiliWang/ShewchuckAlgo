...
public class BinaryTree implements Dictionary {
     ...
     **/
// using comparable, interface that allows u to compare 2 objects
    ...
    /*
    // code provided in Professor Shewchuk's lecture but I like recursion better:
    ...
    */
    ...
    public Entry remove(Object key) {
        ...
        //4 major conditions: no child, 1 child-left, 1 child-right, 2 children
        //3 sub-conditions: node.parent == null, node.parent.leftChild == null, node.parent.rightChild == null

        BinaryTreeNode node = findHelper((Comparable) key, root);
        // declaring a node and check if it's found in the tree
        ...
        if((node.leftChild == null) && (node.rightChild == null)) {
            if(node.parent == null) { // if node is root
                root = null;
            /* if node has no child, detach it from its parent
        ...
        if((node.leftChild != null) && (node.rightChild == null)) {
            if(node.parent == null) { // if node is root; its children becomes root after removal
                root = node.leftChild;
            /* if node has 1 child, move node's child up to take node's place */
            } else if(node.parent.leftChild == node) {
        ...
        // if node has 1 right child (algorithm same with above block)
        if((node.leftChild == null) && (node.rightChild != null)) {
            if(node.parent == null) {
        ...
        if((node.leftChild != null) && (node.rightChild != null)) {
            // look at node's children's subtree
            // specifically the right child of node
            BinaryTreeNode newNode = node.rightChild;
            // find minimum key of that subtree
            // let newNode be in node's right subtree with smallest key
            while(newNode.leftChild != null) {
                newNode = newNode.leftChild;
            }
            Entry temp = node.entry;
            node.entry = newNode.entry;
            // delete newNode and it CAN only has rightChild (itself is smallest)
