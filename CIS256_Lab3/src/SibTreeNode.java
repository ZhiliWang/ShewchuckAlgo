...
class SibTreeNode extends TreeNode {
    ...
    public TreeNode parent() throws InvalidNodeException {
        ...
        if (isValidNode()) {
            if (this == myTree.root) { // only a root's parent is null
                // no such a thing as "parent of a root", so access is not allowed
                // if (this.parent == null)
    ...
    public TreeNode child(int c) throws InvalidNodeException {
        if (isValidNode()) {
            ...
            SibTreeNode kid = firstChild; // why not: SibTreeNode kid = this.firstChild;
            // why not
            // SibTreeNode kid = new SibTreeNode(tree, item);
            // kid = firstChild;
            while ((kid != null) && (c > 1)) {
                kid = kid.nextSibling;
                c--;
